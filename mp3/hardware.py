...
import logging
#Logger:
logging.basicConfig(level=logging.ERROR, format='%(asctime)s - %(levelname)s - %(message)s') #Change level of logging output here
...
class Buttons():
    def __init__(self, frontend):
        ...
        for pin, label in zip(self.PINS, self.LABELS):
            self.BUTTONS.append(Button(pin, pull_up=True, hold_time=1, hold_repeat=True))
